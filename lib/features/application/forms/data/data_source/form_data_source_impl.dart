  ...
  ResultFuture<List<Form>?> getFormsByApplication({
    ...
    try {
      if (applicationId.trim().isEmpty) {
        return Left(APIException(message: "Missing applicationId", statusCode: 400));
      }

      // endpoint: https://backend-tc-sa-v2.onrender.com/api/form/application/:applicationId
      final endpoint = 'https://backend-tc-sa-v2.onrender.com/api/form/application/$applicationId'
      ...
      final list = (raw != null && raw['data'] is List) ? (raw['data'] as List) : <dynamic>[];
      ...
      final forms = list.map<Form>((item) {
        ...
        if (map.containsKey('schoolId')) map['schoolId'] = _norm(map['schoolId']);
        if (map.containsKey('studId')) map['studId'] = _norm(map['studId']);
        if (map.containsKey('applicationId')) map['applicationId'] = _norm(map['applicationId']);
        if (map.containsKey('applicationForm')) map['applicationForm'] = _norm(map['applicationForm']);
  ...
  try {
    ...
    final forms = rawList.map<Form>((raw) {
      ...
      final Map<String, dynamic> item =
          (raw is Map<String, dynamic>) ? Map.from(raw) : {'_id': raw.toString()};

      // Normalize nested fields that Form.fromJson expects as maps
      final schoolObj = _ensureMap(item['schoolId'] ?? item['school']); // some APIs use 'school'
      final studObj = _ensureMap(item['studId'] ?? item['user'] ?? item['stud']); // defensive
      final applicationObj = _ensureMap(item['applicationId'] ?? item['application']);
      final applicationFormObj = _ensureMap(item['applicationForm'] ?? item['applicationFormId']);
      ...
      if (applicationFormObj != null) item['applicationForm'] = applicationFormObj;
...
ResultFuture<Form?> submitForm({
  ...
  try {
    ...
    if (studId.isEmpty) {
      return Left(APIException(message: "Missing student id", statusCode: 400));
    }
...
final raw = result.data as Map<String, dynamic>? ?? {};
    ...
    final Map<String, dynamic> normalized = Map<String, dynamic>.from(unwrapped);

    // If backend returns applicationForm (pdf id), map it to applicationId key expected by your model.
    if (normalized.containsKey('applicationForm') && !normalized.containsKey('applicationId')) {
    ...
    try {
      ...
      return Left(APIException(message: 'Failed to parse form response', statusCode: 500));
    }
  } catch (e, st) {
